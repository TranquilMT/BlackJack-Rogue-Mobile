...
function updateHand(hand: Hand, newCards: Card[], state: GameState): Hand {
    ...
    for (const synergy of updatedHand.activeSynergies) {
        switch (synergy.id) {
            ...
            case SynergyId.RoyalCourt: synergyMultiplier += 0.8; break;
            case SynergyId.PairedAces: updatedHand.status = HandStatus.Blackjack; break;
            ...
            case SynergyId.KingsRansom: updatedHand.flatDamageBonus += 5; break;
            case SynergyId.FullHouse: synergyMultiplier += 3.0; break;
            case SynergyId.Flush: updatedHand.status = HandStatus.Win; synergyMultiplier += 1.0; break;
            case SynergyId.StraightFlush: updatedHand.status = HandStatus.SuperWin; break;
            case SynergyId.TwoPair: synergyMultiplier += 1.8; break;
            case SynergyId.ThreeOfAKind: synergyMultiplier += 2.5; break;
            ...
            case SynergyId.Snapshot: updatedHand.flatDamageBonus += 10; break;
            case SynergyId.FiveCardCharlie: updatedHand.status = HandStatus.Win; break;
            case SynergyId.DoubleDownFrenzy: synergyMultiplier += 0.5; break;
            case SynergyId.SplitUniverse: synergyMultiplier += 0.3; break;
        }
    }
...
function dealInitialHands(state: GameState): GameState {
    ...
    // Check for Oracle reveal
    if (finalPlayerCards.some(c => c.modifier === CardModifierId.THE_ORACLE)) {
        // Oracle reveal logic
    }

    const playerHand = updateHand(emptyHand, finalPlayerCards, tempState);

    // Check for Abyssal Eye reveal
    if (tempState.relics.some(r => r.id === RelicId.AbyssalEye) && playerHand.score <= 10) {
        // Abyssal Eye reveal logic
    }

    const dealerHand = updateHand({ ...emptyHand, id: 0 }, [dCard1, dCard2], tempState);
    ...
    switch (action.type) {
        ...
        case 'HIT': {
            ...
            if (finalState.relics.some(r => r.id === RelicId.AbyssalEye) && updatedHand.score <= 10) {
                finalState.dealerHand.cards = finalState.dealerHand.cards.map(c => ({ ...c }));
            }
        ...
        case 'RESOLVE_HAND': {
            ...
            state.playerHands.forEach(hand => {
                ...
                const result = determineHandResult(hand, state.dealerHand);
                ...
                if (result === HandStatus.Win || result === HandStatus.Blackjack || result === HandStatus.SuperWin) {
                    ...
                    if(hand.activeSynergies.some(s => s.id === SynergyId.SplitUniverse)) {
                    ...
                    if (hand.activeSynergies.some(s => s.id === SynergyId.BlackAndRed)) healing += 2;
        ...
        case 'SELECT_BOON': {
            ...
            switch (action.boonId) {
                ...
                case BoonId.SURVIVAL_BONUS:
                    nextState.survivalDamageMultiplier += 0.1;
                    break;
            }
            return dealInitialHands({ ...nextState, gamePhase: 'roundStart' });
        }
        ...
        case 'USE_POTION': {
            ...
            if (action.potionId === PotionId.HealingPotion) {
                ...
                const revealedCards = newState.dealerHand.cards.map(c => ({ ...c }));
                newState.dealerHand = { ...newState.dealerHand, cards: revealedCards };
                newState.message = "Dealer's hand revealed!";
            } else if (action.potionId === PotionId.RagePotion) {
                newState.isRageActive = true;
                ...
                if (activeHand && activeHand.cards.length > 0) {
                    ...
                    const newHands = [...newState.playerHands];
                    newHands[newState.activeHandIndex] = { ...activeHand, cards: newCards, score: calculateHandScore(newCards) };
