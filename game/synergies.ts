...
export const SYNERGIES: Record<SynergyId, Omit<Synergy, 'id'>> = {
  ...
  [SynergyId.PairedAces]: {
    name: 'Paired Aces',
    description: 'This hand is automatically a Blackjack with the Blackjack damage bonus.',
  },
...
const checkStraightFlush = (hand: Hand): Synergy | null => {
    ...
    const ranks = hand.cards.map(c => RANK_ORDER[c.rank]).sort((a, b) => a - b);
    const isConsecutive = (values: number[]) => values.every((rank, index) => index === 0 || rank === values[index - 1] + 1);
    const aceLowRanks = ranks.map(rank => rank === 14 ? 1 : rank).sort((a, b) => a - b);

    if (isConsecutive(ranks) || isConsecutive(aceLowRanks)) {
